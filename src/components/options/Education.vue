...
export default {
  ...
  mounted () {
    ...
    if(localStorage.getItem('education')){
      this.defaultColumns = JSON.parse(localStorage.getItem('education'))
      this.showData()
    } else {
    dictionaryQuery({type: 'education' }).then((res) => {
      if(res.data) {
        this.defaultColumns = res.data.sort((a, b) => a.value - b.value)
        localStorage.setItem('education', JSON.stringify(this.defaultColumns))
        this.showData()
      }
    })
    }
    ...
    showData () {
      ...
      this.$nextTick(() => {
        if(this.data.length > 0){
          ...
          this.$refs.picker.setColumnIndex(1,  this.dadta[1] === '-1' ? 0 : valuesTwo.findIndex(el => el.value === this.data[1]))
        }
      })
      this.isLoading = false
    },
    onChange (picker, values) {
      picker.setColumnValues(1,values[0] === '不限'? this.unshiftArr(this.maps(this.defaultColumns)) : this.unshiftArr(this.maps(this.ages('label', values[0]))))
    },
    onConfirm (values) {
      let min = values[0] === "不限" ? '-1' : this.defaultColumns.find(el => el.label === values[0])
      let max = values[1] === "不限" ? '-1' :this.defaultColumns.find(el => el.label === values[1])
      this.$emit('confirm','education', [min, max])
    }
  },
  components: {
    Actionsheet,
